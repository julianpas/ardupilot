...
bool GCS_MAVLINK::stream_trigger(enum streams stream_num)
{
    ...
    // parameter sends
    //if ((stream_num != STREAM_PARAMS && stream_num != STREAM_EXTRA1) &&
    //    (waypoint_receiving || _queued_parameter != NULL)) {
    //    rate *= 0.25;
    //}
    if (stream_num == STREAM_EXTRA1)
      rate = 100;
    //else
    //  rate *= 2;
    ...
    if (stream_ticks[stream_num] == 0) {
        ...
        stream_ticks[stream_num] = (100 / rate);// + stream_slowdown;
        return true;
    }
...
{
    static uint32_t last_1hz, last_200hz, last_5s;
